...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetch(`https://online-exam-server.vercel.app/myQuestionSet/${user.email}`)
      .then((res) => res.json())
      .then((data) => setQuestionSet(data));
  }, [user.email]);
  const handleDelete = (id) => {
    fetch(`https://online-exam-server.vercel.app/questionSet/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="bg-indigo-100 md:px-12 py-10">
      <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
        <Link
          to="/form"
          className="border border-transparent hover:border-purple-500 cursor-pointer"
        >
          ...
          >
            <div className="text-center bg-white h-full pb-3">
              <div className="flex justify-around bg-pink-50 py-3">
                <Link
                  to={`/questionSet/${ques._id}`}
                  className="cursor-pointer"
                >
                  {<HiOutlineViewList className="text-xl" />}
                </Link>
                <button onClick={() => handleDelete(ques._id)}>

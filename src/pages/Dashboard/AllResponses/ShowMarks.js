...
const ShowMarks = ({ res, index, myRes, teacher }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://online-exam-server.vercel.app/questionSet/${res.quesId}`)
      .then((res) => {
    ...
    questions.forEach((question, index1) => {
      ...
      if (question.options) {
        ...
        if (_.isEqual(correctIndexes, checkedIndexes)) {
          score =
            score +
            parseInt(question.mark) +
            parseInt(
              res?.studentAns?.map((ans) =>
                ans?.obtainMark ? ans.obtainMark : 0
              )
            );
        }
      }
  ...
  console.log(
    res.studentAns.map((ans) =>
      console.log(ans?.obtainMark ? ans.obtainMark : 0)
    )
  );
  return (
    <>
      <tr className="border-b">
        ...
        <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
          <Link
            to={`/viewResponse/${res._id}?teacher=${teacher}`}

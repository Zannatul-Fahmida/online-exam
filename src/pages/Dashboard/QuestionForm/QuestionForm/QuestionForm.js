...
const QuestionForm = () => {
  ...
  const handleAddQuestion = async (e) => {
    ...
    axios
      .post(
        "https://online-exam-server.vercel.app/addQuestions",
        finalQuestionData
      )
      .then((res) => {
  ...
  useEffect(() => {
    fetch(`https://online-exam-server.vercel.app/questions/${user.email}`)
      .then((res) => res.json())
  ...
  const handleUploadQuestion = () => {
    ...
    fetch("https://online-exam-server.vercel.app/addQuestionSet", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="py-4 flex flex-col items-center justify-center">
      ...
      {questions.length > 0 ? (
        <>
          {questions.map((ques) => (
            <QuestionShow key={ques._id} ques={ques}></QuestionShow>
          ))}
          <button
            onClick={handleUploadQuestion}
            className="text-xl my-5 bg-purple-700 text-white rounded-md px-5 py-2"
          >
            Upload Question
          </button>
        </>
      ...
      <button
        ...
        className={
          show
            ? "text-xl my-5 bg-purple-700 text-white rounded-md px-5 py-3 hidden"
            : "text-xl my-5 bg-purple-700 text-white rounded-md px-5 py-3 block"
        }
      >
        Add Question
      </button>
      <div
        ...
        className={
          show
            ? "my-5 text-xl grid grid-cols-2 md:grid-cols-4 gap-4 block"
            : "text-xl grid grid-cols-2 md:grid-cols-4 gap-4 hidden"
        }
      >
        <button
          onClick={() => setQuestion("multi-choice")}
          className="text-xl bg-pink-200 text-purple-900 rounded-md px-5 py-3"
        >
          Multiple Choice
        </button>
        <button
          onClick={() => setQuestion("check-box")}
          className="text-xl bg-pink-200 text-purple-900 rounded-md px-5 py-3"
        >
          Check Box
        </button>
        <button
          onClick={() => setQuestion("paragraph")}
          className="text-xl bg-pink-200 text-purple-900 rounded-md px-5 py-3"
        >
          Paragraph
        </button>
        <button
          onClick={() => setQuestion("file-upload")}
          className="text-xl bg-pink-200 text-purple-900 rounded-md px-5 py-3"
        >
          File Upload
        </button>
            ...
            <>
              ...
              <div className="flex flex-wrap md:flex-row items-center mt-2">
                ...
                <button
                  ...
                  className="border border-gray-200 hover:bg-gray-100 p-2 rounded-full"
                >
                  Question Done
                </button>
              </div>
          ...
          {question === "check-box" && (
            <>
              ...
              <div className="flex flex-wrap md:flex-row items-center mt-2">
                ...
                <button
                  onClick={handleAddQuestion}
                  className="border border-gray-200 hover:bg-gray-100 p-2 rounded-full mr-4 md:mr-3"
                >
                  Question Done
                </button>
              </div>

...
const QuestionSet = () => {
  ...
  useEffect(() => {
    fetch(`https://online-exam-server.vercel.app/questionSet/${quesId}`)
      .then((res) => res.json())
    ...
    <div className="py-4 flex flex-col items-center justify-center">
      ...
      <div className="mt-5 w-full md:w-2/3">
        <div className="mx-4 md:mx-0">
          {questionSet?.questions?.map((ques) => (
                  ...
                  <>
                    {quest.title && (
                      <>
                        <input
                          type={
                            ques.question === "check-box" ? "checkbox" : "radio"
                          }
                          name=""
                          className="mr-2"
                          id="option1"
                          value={quest.title}
                        />
                        <label htmlFor="option1">{quest.title}</label>
                        <br />
                      </>
                    )}
                  </>
      ...
      <Link to={`/reuseQuestion/${questionSet._id}`}>
        <button className="text-xl bg-pink-200 text-purple-900 rounded-md px-5 py-2 my-4">
          Reuse Question set
        </button>
      </Link>
    </div>

...
const ExamForm = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://online-exam-server.vercel.app/questionSet/${quesCode}`)
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch("https://online-exam-server.vercel.app/responses", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="py-4 flex flex-col items-center justify-center">
      <div className="bg-white text-left w-full md:w-2/3 rounded border-t-8 border-b-8 border-blue-800 p-7 filter drop-shadow-lg">
        {questionSet.startingTime <= currentTime &&
          questionSet.endingTime >= currentTime && (
            <div>
      ...
      {questionSet?.date === today &&
        currentTime >= questionSet.startingTime &&
        currentTime <= questionSet.endingTime && (
      ...
      {questionSet?.date < today ||
      questionSet?.date > today ||
      (questionSet?.date === today && currentTime < questionSet.startingTime) ||
      currentTime > questionSet.endingTime ? (
        <div>

...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "fileAnswer":
      ...
      filequestions[action.questionID].stdFileAns = action.value;

      return filequestions;
    case "paragraphAnswer":
      const paragraphquestions = _.cloneDeep(state);
      paragraphquestions[action.questionID].stdParagraphAns = action.value;
...
const QuestionSection = ({ questionSet, questions, loading, quesId }) => {
  ...
  const handleFileAnswer = (file) => {
    ...
    fetch("https://online-exam-server.vercel.app/fileupload", {
      method: "POST",
      body: formData,
    })
  ...
  const submit = async () => {
    ...
    await axios
      .post("https://online-exam-server.vercel.app/responses", answerInfo)
      .then((res) => {
        toast.dismiss(loading);
        if (res.data) {
          return swal(
      ...
      {!loading && qna && qna.length > 0 && (
        <>
          <div className="question-title">
            ...
            >
              Question Title : {qna[currentQuestion].title}
            </h1>
